...
setTimeout(() => {
    ...
    console.log(deviceStatus.find((item) => item.id == 4));
}, (Math.ceil(deviceIds.length / 5) * 3000));
export async function getDevicesOnlineStatus(deviceIds) {
...
*/
export async function fetchDeviceStatusConcurrentRequest(chunk) {
    const statusdev = await Promise.allSettled(chunk.map(async (deviceId) => {
        ...
        var devStatus = { id: deviceId, status: data.completed ? "online" : "offline" };
        deviceStatus.push(devStatus);
    }));
    return deviceStatus;
}
//# sourceMappingURL=fetch-pq-exercise-with-instructions.js.map